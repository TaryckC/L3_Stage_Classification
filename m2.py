...
####################PTC##############################"""
edgesF= 'PTC_FM/PTC_FM_A.txt'           #'mutag_data/MUTAG_A.txt'
edgesLabelsF = 'PTC_FM/PTC_FM_edge_labels.txt'    #'mutag_data/MUTAG_edge_labels.txt'
#------------------ID GRAPHE super important----------
graphIndsF = 'PTC_FM/PTC_FM_graph_indicator.txt'       #'mutag_data/MUTAG_graph_indicator.txt'
#---------------------------------------------------------
graphLabelsF = 'PTC_FM/PTC_FM_graph_labels.txt'     #'mutag_data/MUTAG_graph_labels.txt'
nodesLabelsF = 'PTC_FM/PTC_FM_node_labels.txt'    #'mutag_data/MUTAG_node_labels.txt'
...
def SgExtractor(graph_id, fileToPyObject,sgSize=-1):
    ...
    #nodeList = INS.get_GraphNodes(graph_id)
...
###test
'''graphNodesRes= INS.get_GraphNodes(90)

g=create_Graph(graphNodesRes, INS.get_GraphEdges(graphNodesRes))
sG=SgExtractor(90, 2)
...
'''GM = isomorphism.GraphMatcher(g,sG,node_match= lambda n1,n2 : n1['atome']==n2['atome'], edge_match= lambda e1,e2: e1['label'] == e2['label'])

print(GM.subgraph_is_isomorphic())'''

#maintenant il nous faut une fonction qui fasse la verif plusieurs fois


# avec id l'id du graph connu, id2  l'id du graphe duquel on va extraire les ss graphes et n le nombre de noeuds du ss graphe extrait
#Tps de calc pour graphs qui matchent
def diagMaker(id,id2,n):
    graphNodesRes= INS.get_GraphNodes(id)
    g=create_Graph(graphNodesRes, INS.get_GraphEdges(graphNodesRes),INS)
    timeL=[]
    timeL2=[]
    ResL=[]
    ResL2=[]
    ...
    for j in range (1,n):
        ...
        sG=SgExtractor(id2,INS, j)
        GM = isomorphism.GraphMatcher(g,sG,node_match= lambda n1,n2 : n1['label']==n2['label'], edge_match= lambda e1,e2: e1['label'] == e2['label'])
        ISO=GM.subgraph_is_isomorphic()
        # on rep l'opération pour avoir une sorte de moyenne et améliorer la précision
        if ISO == True:
            
            '''for w in range(99):
                sG=SgExtractor(101, j)
                GM = isomorphism.GraphMatcher(g,sG,node_match= lambda n1,n2 : n1['atome']==n2['atome'], edge_match= lambda e1,e2: e1['label'] == e2['label'])
                ISO=GM.subgraph_is_isomorphic()'''
...
def random_inator():
    numbers = set()
    while len(numbers) < 3:
        numbers.add(random.randint(1, 300))  
    return list(numbers)

#lrand=random_inator()
#res =diagMaker(lrand[0],lrand[1],lrand[2])
# Res non aléat
res =diagMaker(85,90,10)
#Résultat : TypeError: create_Graph() missing 1 required positional argument: 'fileToPyObject'




# Tps de calcul pour ssGraphes qui matchent en fonction du nombres de noeuds du ss graphe
plt.plot(res[2], res[0], marker='o', linestyle='-', color='b')
# pour ssGraphes qui ne matchent pas 
plt.plot(res[3], res[1], marker='o', linestyle='-', color='r')
plt.show() 
#####
    
#####
#####réparer le code aléat, utiliser avec une autre base