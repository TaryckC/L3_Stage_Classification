...
class FileToPy :
    ...
    def __init__(self, edgesF, edgesLabelsF, graphIndsF, graphLabelsF, nodesLabelsF) :
        ...
        #Enregistrement de tous les graphes dans une liste et association de leur classe grace à l'attribut NetworkX.
        self.GRAPH_LIST = []
        for i in range (self.nbGraphe) :
            self.GRAPH_LIST.append(self.create_Graph(i))
    ...
    #Fonction de création d'un graphe NetworkX :
    def create_Graph(self, id):
        #Récupération des noeuds et des arêtes pour un graph.
        nodes_list = self.get_GraphNodes(id)
        edges_list = self.get_GraphEdges(nodes_list)


        G = nx.Graph(label=self.graphLabels[id])
        nodeslbls_list = self.node_to_Label(nodes_list)
        for i in range(len(nodes_list)):
            G.add_node(nodes_list[i], label=nodeslbls_list[i])

        edgeslbls_list = self.edgelist_to_label(edges_list)
    ...
    # retourne le nombre de graphes dans la base 
    def get_num_graphs(self):
        return max(self.graphIndicators)
