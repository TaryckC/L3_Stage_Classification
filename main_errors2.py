...
## modif de sg extractor qui me permet de choisir mon nb de noeuds 
def SgExtractor2(graph_id, nodesLabels_filename, edgesLabels_filename, edges_filename, graphInds_filename, nbNoeudsListe,len):
    ...
    # Taille du sous-graphe choisi PAS aleatoirement 
    # len en paramètre 
    SgLen = len 
    # problème ici !
    nbNoeudsListe.append(SgLen+nbNoeudsListe[len(nbNoeudsListe)-1])
...
def generate_random_subgraphs(nodesLabels_filename, edgesLabels_filename, edges_filename, graphInds_filename,nbNoeudsListe,len):
    # Choix aléatoire d'un graphe dans la base de données
    random_graph_id = random.choice(graphInds_filename)

    # Initialisation de la liste des sous-graphes
    subgraphs = []

    # Génération de 10 sous-graphes
    for _ in range(10):
        
        # Appel de la fonction SgExtractor2 pour extraire un sous-graphe
        subgraph = SgExtractor2(random_graph_id, nodesLabels_filename, edgesLabels_filename, edges_filename, graphInds_filename,nbNoeudsListe,len)
        # Ajout du sous-graphe à la liste
        print("eh",subgraphs)
...
# On va tenter une modification du prédicateur 
...
# predicateur qui va avec sg2
def predicateur2(graph_id, n, sousBaseTrain, minCf, minGrowth, nbNoeudsListe, listeTempsNoeuds, graphMatch, tempsGraphMatch, graphNonMatch, tempsGraphNonMatch):
    ...
    for i in range(n):
        ...
        nodeList = get_GraphNodes(graph_id ,get_GraphIndicator(graphInds_filename))
        # on entre une valeur aleatoire entre 1 la la longueur max dans len
        k = generate_random_subgraphs(nodesLabels_filename, edgesLabels_filename, edges_filename, graphInds_filename, nbNoeudsListe,random.randint(1, len(nodeList)))
        
        # predict prend comme valeur la classe supposé du graphe
        #Calcule les mesures confiance etc
        # k donne une liste ( si tout va bien ) on modifie la fonction pour gérer les listes 
        for graph in k :
            time_exec, predict, cf, growthR = noyeauPrediction(sousBaseTrain, k, graphsLabels_filename, graphMatch, tempsGraphMatch, graphNonMatch, tempsGraphNonMatch)
...
def prediction_loop(ntm, ntp, res) :
    ...
    for i in range (0,len(ntp)):
        ...
        # on change predicateur en predicateur 2
       # r,t= predicateur(ntp[i], 20, ntm, 0.7, 2, nbNoeudsListe, listeTempsNoeuds, graphMatch, tempsGraphMatch, graphNonMatch, tempsGraphNonMatch)#10 
        r,t= predicateur2(ntp[i],20, ntm, 0.7, 2, nbNoeudsListe, listeTempsNoeuds, graphMatch, tempsGraphMatch, graphNonMatch, tempsGraphNonMatch)
        if r[0] == r[1]:
            testOk += 1
        elif r[0] == 0:
            testZ += 1
